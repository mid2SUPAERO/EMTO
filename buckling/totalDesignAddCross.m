function [totalMat]=totalDesignAddCross(xdens,xcos,xsin,xcub,nelx,nely,totvolfrac,posttreat,problem,Fval,xSize,E0,nEig)
...
if posttreat==1;
    totalMat=posttreatment(totalMat,nelx,nely,microx,microy,totvolfrac,problem,Fval,xSize,E0,nEig,xdens,xcos,xsin);
end
end

function totalMat01=posttreatment(totalMat,nelx,nely,microx,microy,totvolfrac,problem,Fval,xSize,E0,nEig,xdens,xcos,xsin)
nelxm=nelx*microx;
nelym=nely*microy;
...
%%add crosses
%find buckling cells
topleftmat= totalMat01(1:2:end,1:2:end);
toprightmat= totalMat01(1:2:end,2:2:end);
botleftmat= totalMat01(2:2:end,1:2:end);
botrightmat= totalMat01(2:2:end,2:2:end);
halftotmat=(topleftmat+toprightmat+botleftmat+botrightmat)./4;  
[blf,mu,eivSort]=evalBuc(halftotmat,nEig,problem,Fval,xSize,E0);

eivtot=zeros(nely,nelx);
for k=1:12
    modeN=k;
    nelym=microy/2*nely;
    nelxm=microx/2*nelx;
    firstCol=[zeros(1,nelym+1);eivSort(1:nelym+1,modeN)'];
    eiv1=[firstCol(:);eivSort(nelym+2:end,modeN);0];
    eiv1H=eiv1(1:2:end);
    eiv1V=eiv1(2:2:end);
    eiv1H=reshape(eiv1H,nelym+1,nelxm+1);
    eiv1V=reshape(eiv1V,nelym+1,nelxm+1);
    eiv1N=(eiv1H.^2+eiv1V.^2).^0.5;
    htmlarger1=[[halftotmat,zeros(nelym,1)];zeros(1,nelxm+1)];
    htmlarger2=[[zeros(nelym,1),halftotmat];zeros(1,nelxm+1)];
    htmlarger3=[zeros(1,nelxm+1);[halftotmat,zeros(nelym,1)]];
    htmlarger4=[zeros(1,nelxm+1);[zeros(nelym,1),halftotmat]];
    htmlarger=(htmlarger1+htmlarger2+htmlarger3+htmlarger4)/4;
    eivDens=eiv1N.*[[halftotmat,zeros(nelym,1)];zeros(1,nelxm+1)];
    %maxN=max(max(eiv1N));
    %eiv1N=eiv1N./maxN;
    %figure(7)
    %colormap(gray); imagesc(1-eiv1N); caxis([0 1]); axis equal; axis off; drawnow;
    eiv1e=zeros(nely,nelx);
    for i=1:nely
        for j=1:nelx
            eiv1e(i,j)=mean(mean(eivDens(microy/2*(i-1)+1:microy/2*i+1,microx/2*(j-1)+1:microx/2*j+1)));
            %eiv1e(i,j)=eiv1e(i,j)/mean(mean(halftotmat(microy/2*(i-1)+1:microy/2*i,microx/2*(j-1)+1:microx/2*j)));
        end
    end
    eiv1em=eiv1e./max(max(eiv1e));
    %figure(k+6)
    %colormap(gray); imagesc(1-eiv1em); caxis([0 1]); axis equal; axis off; drawnow;
    eivtot=eivtot+eiv1e*mu(k);
end
eivtot=eivtot./max(max(eivtot));

figure(20)
colormap(gray); imagesc(1-eivtot); caxis([0 1]); axis equal; axis off; drawnow;
%figure(21)
%colormap(gray); imagesc(1-eivtotd); caxis([0 1]); axis equal; axis off; drawnow;
eivtotL=repelem(eivtot,microy/2,microx/2);
figure(100)
colormap(gray); imagesc(1-max(eivtotL>0.4,halftotmat)); caxis([0 1]); axis equal; axis off; drawnow;

%add crosses
crosses=totalDesignAddCross(xdens/2.*(eivtot>0.4),xcos,xsin,ones(nely,nelx),nelx,nely,totvolfrac,0,problem,Fval,xSize,E0,nEig);
totalMat=min(1,totalMat01+crosses);
...
botrightmat= totalMat(2:2:end,2:2:end);
halftotmat=(topleftmat+toprightmat+botleftmat+botrightmat)./4;  
[stress]=totalFEM_S(nelxm,nelym,halftotmat,problem);
stressTotMat3=abs(stress).*halftotmat;
meanSTM3=mean(mean(stressTotMat3));
totalMat(repelem(stressTotMat3<meanSTM3/unusedRatio4,2,2))=0;
